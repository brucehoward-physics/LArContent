...
namespace lar_content {
  ...
  MvaPfoCharacterisationAlgorithm<T>::IsClearTrack(const Cluster* const pCluster) const
  {
    ...
    const LArMvaHelper::MvaFeatureVector featureVector(
      LArMvaHelper::CalculateFeatures(m_featureToolVector, this, pCluster));
    ...
    if (!m_enableProbability) { return LArMvaHelper::Classify(m_mva, featureVector); }
    else {
    ...
    const pandora::ParticleFlowObject* const pPfo) const
  {
    ...
    const LArMvaHelper::StringVector chosenFeatureToolOrder(
      ...
      if (isMainMCParticleSet) {
        if (completeness >= 0.f && purity >= 0.f && !mischaracterisedPfo &&
          ...
          LArMvaHelper::ProduceTrainingExample(
            outputFile, isTrueTrack, chosenFeatureToolOrder, featureMap);
        }
      }
    ...
    if (!m_enableProbability) {
      return LArMvaHelper::Classify(
        (wClusterList.empty() ? m_mvaNoChargeInfo : m_mva), chosenFeatureToolOrder, featureMap);
    }
    else {
